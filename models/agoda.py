...
class AgodaAccount:
    def __init__(self, cookie_file, proxy):
        ...
        self.session = httpx.AsyncClient(transport=transport, timeout=120)
        # self.session = httpx.AsyncClient(proxies=proxies, timeout=30)
        self.session.follow_redirects = True
        self.session.headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:123.0) Gecko/20010101 Firefox/123.0',
            'Accept': '*/*',
            'Accept-Language': 'en-US,en;q=0.5',
            'Referer': 'https://ycs.agoda.com/mldc/en-us/app/reporting/booking/399323?bookingType=confirmed',
    ...
    async def get_reservations(self):
        ...
        response = await self.session.post(f'https://ycs.agoda.com/en-us/{self.hotel_id}/kipp/api/hotelBookingApi/Get/', data=data)
        ...
        for i in reservations:
            if i['AckRequestType'] == 1:
                bookingIds.append({"bookingId": str(i['BookingId'])})

        url = f'https://ycs.agoda.com/en-us/{self.hotel_id}/kipp/api/hotelBookingApi/GetDetails'
        bookingIds_str = str(bookingIds).replace('[', '').replace(']', '')

        response = await self.session.post(url, data={'BookingDetailList': f'[{bookingIds_str}]'})
        get_details_raw = response.json()['Data']
        
        rate = False
        for data_raw in get_details_raw:
        ...
        ready_data = []
        data = {
            'externalId': int(self.hotel_id),
            'bookingAndPropertyDetails': bookingIds,
            'isMobApp': False,
            'languageId': 1,
    ...
    async def send_message(self, message, conversationId, memberId, bookingId, checkIn, checkOut, guestName, hotelName):
        self.session.headers['Content-type'] = 'application/json; charset=utf-8'
